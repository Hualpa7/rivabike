  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-[1120px] px-6">
        <div className="rounded-card bg-paper/95 p-6 shadow-soft backdrop-blur-sm md:p-10">
          <SectionHeading
            ...
            {steps.map((step, i) => (
              <Reveal key={step.n} from="up" delay={i * 0.09}>
                <div className="border-t border-line pt-7">
                  <span
                    className={cn(
                      'inline-flex h-10 min-w-10 items-center justify-center rounded-pill px-3 font-mono text-sm font-bold',
                      step.accent ? 'bg-pink text-white' : 'bg-ink text-white',
                    )}
                  >
                    {step.n}
                  </span>
                  <h3 className="mt-4 font-display text-[21px] font-semibold leading-tight text-ink">
                    {step.title}
                  </h3>
